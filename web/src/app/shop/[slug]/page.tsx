...
import { getDiscountedPrice } from "@/lib/getDiscountedPrice"; // 👈 thêm dòng này
...
export default async function ProductDetailPage(
  ...
  const finalPrice = getDiscountedPrice(product); // 👈 giá sau giảm
          ...
          {product.discount ? (
            <div className="mt-4">
              ...
              <p className="text-sm text-green-600 mt-1">
                Giảm {product.discount}% (tiết kiệm {formatVND(product.price - finalPrice)})
              </p>
            </div>
          ) : (
